 ...
 * @details prerequisites. The caller of this library should have initialized
 * the corresponding semaphores and has already initialized a shared memory
 * object. Since a circular buffer is nothing more than ana array with
 * positional read/write heads, limited via the modulo operator
 * @version 0.1
 * @date 2026-09-18
 *
...
readCircularBuffer()
{
	...
	circularBuffer->readhead = (circularBuffer->readhead + 1) %
	    MAX_BUFF_SIZE;
	return rv;
}
